...
import { getDiscountCodeByCode } from '../../service/apiDiscountCode';
...
const PaymentModal: React.FC<Props> = ({ open, onClose, appointment, onUpdateSuccess }) => {
    ...
    const handleApplyDiscount = async () => {
        ...
        const dcode = await getDiscountCodeByCode(discountCode);
        setTimeout(() => {
            if (dcode != '') {
                const discountAmount = appointment.totalPrice * Number(dcode.discountAmount / 100); // Giảm 10%
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm">
            <div className="bg-white w-full max-w-2xl max-h-[90vh] overflow-y-auto p-6 rounded-xl shadow-lg relative sm:m-0 m-2">
                <button
                    onClick={onClose}
